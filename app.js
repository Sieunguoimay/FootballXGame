...
function getUserInfo(){
	...
	//var base_url = "https://dev-api.ttab.me";
...
function sendScore(){
	...
	for(var i = 0;i <loaded_score_data.length; i++){
		corrected_data.push(loaded_score_data[i]);

		var new_record = [];
		if(loaded_score_data[i].records)
		for(var j = 0; j<loaded_score_data[i].records.length; j++){
			var existed = false;
			for(var k = 0; k<new_record.length; k++)
				if(loaded_score_data[i].records[j].s===new_record[k].s
					&&loaded_score_data[i].records[j].t===new_record[k].t){
					existed = true;
				}
			if(!existed){
				new_record.push(loaded_score_data[i].records[j]);
			}
			total_play++;
		}

		corrected_data[corrected_data.length-1].records = [];
		corrected_data[corrected_data.length-1].count = new_record.length;
		Object.assign(corrected_data[corrected_data.length-1].records,new_record);
	}
	//send this score to somewhere please.
	myFBref.set(corrected_data);
...
var teams = {
	...
	*/
	...
	num_loaded_logos:0,
	loadLogo:function(name,src){
		this.eleven_club_logos[name] = new Image();

		this.eleven_club_logos[name].onload = function(){
			this.num_loaded_logos += 1;
			if(this.num_loaded_logos===11){
				////////loaded successfully///////
				//logo ready to use
				////////come on babe /////////////
			}
		}
		this.eleven_club_logos[name].src = src;

	},
	load_eleven_logos:function(){
		this.eleven_club_logos = {};
		this.num_loaded_logos = 0;
		for(var i = 0 ; i< gamedata.clubs.length; i++){
			for(var j = 0; j<this._teams[level].positions.length;j++)
			if(gamedata.clubs[i].name === this._teams[level].positions[j])
				this.loadLogo(gamedata.clubs[i].name,gamedata.clubs[i].club_logo);
		}
	},
	...
	draw:function(position,x,y){
		...
		ctx.shadowColor = '#fff';
		ctx.shadowBlur = 50;
		if(this.eleven_club_logos[img]){
			var w = h*(this.eleven_club_logos[img].width/this.eleven_club_logos[img].height);
			drawImage3(this.eleven_club_logos,img,x,y-h/4,w+this.breadth,h+this.breadth);
		}
		ctx.shadowBlur = 0;
	}
};
...
function Button(image,x,y,flip=false,breadthEnabled=false,blur = true){
...
var splash = {
	...
	matchButton:new Button(text.matches_txt+' Button',333,1275,false, false, false),
	chatButton:new Button(text.chat_txt+' Button',WIDTH-333,1275,false, false, false),
	...
	onpress:function(x,y){
		...
		this.matchButton.press(x,y);
		this.chatButton.press(x,y);
	},
	onmousemove:function(x,y){
		...
		this.matchButton.hover(x,y);
		this.chatButton.hover(x,y);
	},
	onrelease:function(){
		...
		this.matchButton.release();
		this.chatButton.release();
	},
	init:function(){
		...
		promptScenes["Leaderboard"].endGame = false;
	...
	update:function(){
		...
		this.matchButton.update();
		this.chatButton.update();
...
/*			userLang = ((userLang==='en-US')?'vi-VN':'en-US');
			push_user_language();
			sendScore();
			location.reload();
*/		}
		...
		if(this.matchButton.isTriggered()){
			window.open('https://footballx.live/matches');
		}
		if(this.chatButton.isTriggered()){
			window.open('https://footballx.live/');
		}
	}
	,draw:function(){
...
//		this.displayButton0.draw();
		...
		drawText(text.play_now_txt,WIDTH/2-481,HEIGHT/2+430,'#c1141b',
			80*this.playButton.scale+this.playButton.breadth);
//		this.languageButton.draw();
		this.leaderboardButton.draw();
		this.shareButton.draw();
		this.tipButton.draw();

		this.matchButton.draw();
...
function sound(src) {
    ...
    this.sound.src = src;
    this.sound.setAttribute("preload", "auto|metadata|none");
...
function loadAsset(){
	...
	loadImage('Chat Button');
	loadImage('Match Button');
...
function main(){
	...
	let resizeCanvas = function(){
		...
		CANVAS_HEIHGT = window.innerHeight;
...
function render(){
	...
	if(currentState ===states.Game||currentState === states.Hint||currentState===states.Level){
		if(currentState===states.Level){
			if(displayLevel.txt!=text.stage_txt)
...
function drawImage3(imgs,name,x,y,w,h){
	if(w==null) w = imgs[name].width;
	if(h==null) h = imgs[name].height;
	ctx.drawImage(imgs[name],x-w/2,y-h/2,w,h);	
}
